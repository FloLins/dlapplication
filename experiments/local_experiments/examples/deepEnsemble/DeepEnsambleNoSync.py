...
from DLplatform.synchronizing import NoSync
...
if __name__ == "__main__":
    ...
    sync = NoSync()
    syncPeriod = 1
    indices = 'parallelRun' #or 'roundRobin'
    ...
    dsFactory = FileDataSourceFactory(filename="../../../../data/textualMNIST/mnist_train.txt", decoder=MNISTDecoder(),
                                      numberOfNodes=numberOfNodes, indices=indices, shuffle=False, cache=False)
    learnerFactory = PytorchLearnerFactory(network=MnistNet(), updateRule=updateRule, learningRate=learningRate,
                                           learningParams=learningParams, lossFunction=lossFunction,
                                           batchSize=batchSize, syncPeriod=syncPeriod)
    ...
    exp.run("MNISTtorchCNNforDeeoEnsemblewithNoSync")
